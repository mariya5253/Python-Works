...
def get_simple_rnn_model():
    ...
    model = Sequential() 
    model.add(Embedding(MAX_NB_WORDS, embedding_vector_length, input_length=MAX_LENGTH)) 
    model.add(LSTM(100)) 
...
# rnn_simple_model.fit(padded_train_sequences, y_train, validation_data=(padded_test_sequences, y_test), nb_epoch=3, batch_size=64) 

# scores = rnn_simple_model.evaluate(padded_test_sequences, y_test, verbose=0) 
# print("Accuracy: %.2f%%" % (scores[1]*100))

seed = 7
np.random.seed(seed)
model_d2v_02 = Sequential()
model_d2v_02.add(Dense(64, activation='relu', input_dim=MAX_LENGTH))
...
model_d2v_02.fit(padded_train_sequences, y_train,
                 validation_data=(padded_test_sequences, y_test),
